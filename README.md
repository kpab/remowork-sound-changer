...
### 会議メモ・文字起こし
- **リアルタイム文字起こし**（Chrome/Edge対応、Web Speech API）
- **AI自動構造化メモ**（Gemini/OpenAI/Claude対応）
  - 議題・決定事項・アクションアイテムを自動抽出
  - ツリー構造（├─ └─）で見やすく整理
  - 30秒ごとの自動構造化（レート制限対応）
- **手動メモ**入力欄
- **コピー機能**（個別/全てコピー/履歴からコピー）
- **プロフィール情報**による漢字変換精度向上
- 録音と連動して自動保存
...
録音中に自動で文字起こしとAI構造化メモを生成します。

#### 初期設定
1. ポップアップの「🤖 AI」タブを開く
2. AI構造化メモをONにする
3. LLMプロバイダーを選択（Gemini推奨・無料）
4. APIキーを入力
5. プロフィール情報を入力（任意・漢字変換精度向上）

#### 使い方
1. 録音モーダルを開く（🎙️ボタン）
2. 録音を開始すると自動で文字起こし開始
3. 手動メモも同時に入力可能
4. 30秒ごとにAIが自動構造化（レート制限内で）
5. 各セクションの📋ボタンでコピー
6. 「📋 全てコピー」で一括コピー
...
| プロバイダー | 料金 | 備考 |
|-------------|------|------|
| Gemini | 無料枠あり | 15 RPM / 250 RPD |
| OpenAI | 有料 | GPT-4o, GPT-4o-mini |
| Claude | 有料 | Claude 3.5 Sonnet等 |
| カスタム | - | OpenAI互換API |

> **注意**: 文字起こしはChrome/Edge専用です（Web Speech API）。Braveなど一部ブラウザでは利用できません。
...
© 2025-2026 Thousand Wings. All rights reserved.
